...
\begin{table}[h!]
	...
	\begin{tabular}{c l l}
		...
		2 & Beacon ID (\texttt{0x01}--\texttt{0xFF}) & 1 \\
		3--4 & Sequence number (uint16, \mbox{LE}) & 2 \\
		5 & \texttt{TX} power (dBm, uint8) & 1 \\
		6 & Modulation code: \texttt{0x01}=LoRa, \texttt{0x02}=\FSK{}, \texttt{0x03}=\OOK{} & 1 \\
...
\labfigure{LH1F_fsm_diagram.png}{0.8\textwidth}{Beacon \FSM{}: \texttt{INIT} $\to$ \texttt{CONFIG} $\to$ \texttt{IDLE} $\to$ \texttt{BUILD} $\to$ \texttt{TX} $\to$ \texttt{LOG} $\to$ \texttt{IDLE}, with a \texttt{FAULT} branch off the \texttt{TX} timeout path.}{fig:lh1f-fsm}

\labfigure{LH1F_packet_layout.png}{0.9\textwidth}{Byte-level 32-byte beacon packet layout. Fields are color-coded to match the format table.}{fig:lh1f-packet}
...
\begin{enumerate}[leftmargin=2em]
	\item Implement \texttt{build\_beacon\_packet(uint8\_t *buf, uint16\_t seq, uint8\_t beacon\_id, uint8\_t mod\_code, uint8\_t sf, uint8\_t bw\_code, int8\_t tx\_power\_dbm)} that fills bytes 0--29 according to the packet format (preamble \texttt{0xAE5A} at 0--1, ID at 2, sequence at 3--4, TX power at 5, modulation code at 6, SF at 7, bandwidth code at 8, timestamp at 9--12, payload at 13--29), computes the \CRC{}-16 over bytes 0--29, and writes it MSB-first to bytes 30--31.
	\item Modulation code is fixed per beacon build: \texttt{0x01}=LoRa, \texttt{0x02}=\FSK{}, \texttt{0x03}=\OOK{}. These codes are the canonical ground-truth labels used by the Module~3 datasets and the Module~4 classifier --- do not invent a 0/1 alias.
...
\labfigure{LH1F_scope_pulse.png}{0.8\textwidth}{Oscilloscope capture of the \texttt{TX}-entry \mbox{GPIO} instrumentation pulse. The period should be rock-steady at the configured slot interval ($\pm$5\,ms).}{fig:lh1f-scope}
...
\labfigure{LH1F_dual_beacon_log.png}{0.85\textwidth}{Annotated receiver log during a dual-beacon run: Beacon~A (ID \texttt{0x01}) and Beacon~B (ID \texttt{0x02}) interleaved, each with its own contiguous sequence counter.}{fig:lh1f-duallog}
