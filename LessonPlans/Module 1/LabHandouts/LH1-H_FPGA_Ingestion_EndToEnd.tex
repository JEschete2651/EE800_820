...
\begin{enumerate}[leftmargin=2em]
	...
	\item \texttt{PAYLOAD}: pop 41 more bytes into an internal register bank. Byte index 1 of the original beacon packet (which is offset 3 of the forwarding frame --- start delimiter 0, length 1, beacon bytes start at 2) is the beacon \mbox{ID}. Capture it into a register \texttt{beacon\_id[7:0]}.
	\item \texttt{DONE}: assert a \texttt{new\_frame} pulse for one cycle, then return to \texttt{IDLE}.
\end{enumerate}
For Week~4 scope, ignore the \CRC{}-16 check inside the \FPGA{}. Module~2 will add that.
