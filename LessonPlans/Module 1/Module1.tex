...
\begin{document}
	...
	\begin{table}[ht!]
		...
		\begin{tabular}{l l l}
			...
			Clock Phase (CPHA) & 1 Edge & Module~1 reference configuration (LH1-C bring-up) \\
	...
	\begin{tcolorbox}[infobox={Recommendation}]
		Begin with \UART{} at 115200 baud for initial integration. With 8N1 framing, the theoretical byte rate is 11{,}520\,B/s, so a 43-byte forwarding frame yields approximately 268 frames per second before firmware overhead --- comfortably above the operating envelope of the two-target ping-pong configuration plus interleaved PAUSE captures. Transition to \SPI{} only when scaling beyond three concurrent stations or when latency requirements tighten in later modules.
	\end{tcolorbox}
	...
	\begin{tcolorbox}[infobox={Exercise: End-to-End Data Flow Validation}]
		...
		\begin{enumerate}
			...
			\item Hold the user button on the interceptor and verify the targeted Target's USART2 prints \texttt{"I'm Jammed"} within 100\,ms; on the seven-segment display, expect the alternating \texttt{01}/\texttt{02} pattern to briefly fall silent and then resume after release.
			\item For Module~1 bring-up, note that FPGA ingestion is a delimiter/length path without \CRC{} reject logic; perform the deliberate-corruption \CRC{} rejection test in Module~2 after \texttt{pkt\_parser} \CRC{} checks are implemented.
		\end{enumerate}
	\end{tcolorbox}
