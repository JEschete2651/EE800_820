% Shared acronym macro container for lesson plan modules.
...
%   \makenoidxglossaries
%   \input{../glossary}   (or ../../glossary from LabHandouts)
% Print used acronyms near end of document:
%   \printnoidxglossary[type=\acronymtype,title={Glossary}]

% ── Custom glossary style: right-aligned page list ───────────
% Three-column longtable layout: acronym | expansion | pages.
% The last column is right-justified so page numbers sit flush
% against the right text margin instead of trailing the
% description inline. Requires \usepackage{longtable} and
% \usepackage{array} — both are pulled in automatically by the
% glossary-long style that ships with the glossaries package.
\newglossarystyle{glossrightloc}{%
	\renewenvironment{theglossary}%
		{\begin{longtable}{@{}p{0.11\textwidth} p{0.62\textwidth} >{\raggedleft\arraybackslash}p{0.20\textwidth}@{}}}%
		{\end{longtable}}%
	...
	\renewcommand*{\subglossentry}[3]{%
		 & \glossentrydesc{##2}\glspostdescription{} & ##3\tabularnewline
	}%
	\renewcommand*{\glsgroupskip}{}%
}
\setglossarystyle{glossrightloc}
...
\newacronym{clt}{CLT}{Command Line Tools (STM32Cube)}
...
\providecommand{\CDC}{\gls{cdc}}
\providecommand{\CLT}{\gls{clt}}
