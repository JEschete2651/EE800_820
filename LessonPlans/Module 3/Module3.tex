...
\begin{document}
	...
	\begin{tcolorbox}[objective={Module 3 Learning Objectives}]
		...
		\begin{enumerate}[label=\textbf{LO-\arabic*}]
			...
			\item Implement a host-side Python data collection script that receives feature vectors from the \FPGA{}, attaches ground-truth labels (including the \texttt{pkt\_type} ground truth that decodes the bit-field-extracted packet type), and stores the dataset in a format ready for machine learning ingestion.
	...
	\begin{itemize}[leftmargin=2em]
		...
		\item Implement a host-side Python script that receives feature vectors from the \FPGA{} via \UART{}, decodes the bit-field-packed \texttt{pkt\_type} column, and stores rows with ground-truth labels.
	...
	The \FPGA{}'s \UART{} TX export module (Session~7) transmits feature vectors using a simple framed protocol:
	...
	\begin{table}[ht!]
		...
		\begin{tabularx}{\textwidth}{l c c X}
			...
			\midrule
			Start Marker & 0 & 1\,B & \texttt{0xFE} (distinct from the \texttt{0x7E} interceptor forwarding frame delimiter) \\
			Feature Vector & 1 & 32\,B & Complete feature vector (Module~2 \texttt{tab:feature\_vector} format, including \texttt{pkt\_type} at offset~3 and PAUSE-only fields at offsets~28--29) \\
	...
	\begin{table}[ht!]
		...
		\begin{tabularx}{\textwidth}{l l X}
			...
			\texttt{beacon\_id} & Feature vector & \FPGA{}-parsed Beacon~ID (sender) \\
			\texttt{mod\_kind} & Feature vector & Modulation kind (offset~1; upper 6 bits of radio packet \texttt{Modulation Code} byte) \\
			\texttt{pkt\_type} & Feature vector & Packet type (offset~3): \texttt{0}=DATA, \texttt{1}=ACK, \texttt{2}=PAUSE \\
			...
			\texttt{rx\_timestamp} & Feature vector & Interceptor-side timestamp (ms) \\
			\texttt{sender\_timestamp} & Feature vector & Sender-side timestamp (ms) \\
			\texttt{seq\_num} & Feature vector & Sequence number \\
			\texttt{payload\_crc\_ok} & Feature vector & Radio packet \CRC{} status (1/0) \\
			...
			\texttt{target\_id\_if\_pause} & Feature vector & Offset~28; PAUSE rows only: which Beacon~ID was targeted by the Threat \\
			\texttt{pause\_duration\_units} & Feature vector & Offset~29; PAUSE rows only: hold-off in 100\,ms units \\
