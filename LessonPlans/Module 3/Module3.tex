...
\begin{document}
	...
	\begin{titlepage}
		...
		\begin{tcolorbox}[colback=lightgray, colframe=darkgray, width=0.85\textwidth, arc=2pt]
			...
			Students connect the STM32 beacon--receiver subsystem (Module~1) with the FPGA signal
			...
			that produce the labeled RF dataset for the machine learning pipeline in Module~4.
		\end{tcolorbox}
	...
	\begin{tcolorbox}[objective={Module 3 Learning Objectives}]
		...
		\begin{enumerate}[label=\textbf{LO-\arabic*}]
			...
			\item Measure and characterize end-to-end system performance metrics: packet error rate, end-to-end latency (beacon TX to feature vector availability), sustained throughput, and FIFO utilization under load.
	...
	\begin{itemize}[leftmargin=2em]
		...
		\item Completed Module~2 with a fully simulated FPGA pipeline (UART RX $\rightarrow$ parser $\rightarrow$ CRC $\rightarrow$ feature extraction $\rightarrow$ BRAM), a generated bitstream, and a verified constraint file (\texttt{.xdc}).
	...
	\begin{figure}[ht!]
		...
		\resizebox{0.85\textwidth}{!}{%
			\begin{tikzpicture}[
				...
				\node[io] (uart) {USB-UART\\(COM port)};
				\node[block, right=of uart] (serial) {PySerial\\Reader};
				\node[block, right=of serial] (parse) {Feature Vector\\Parser};
				\node[block, right=of parse] (label) {Label\\Attacher};
				\node[io, right=of label] (csv) {CSV File\\(Dataset)};

				\node[io, below=1cm of label] (config) {Campaign\\Config\\(JSON)};

				\draw[arrow] (uart) -- (serial) node[midway, above, elabel] {raw bytes};
				\draw[arrow] (serial) -- (parse) node[midway, above, elabel] {32B vectors};
				\draw[arrow] (parse) -- (label) node[midway, above, elabel] {parsed fields};
				\draw[arrow] (label) -- (csv) node[midway, above, elabel] {labeled rows};
				\draw[arrow] (config) -- (label) node[midway, right, elabel] {ground-truth\\labels};
			\end{tikzpicture}%
		}
	...
	\begin{table}[ht!]
		...
		\begin{tabularx}{\textwidth}{l l X}
			...
			STM32CubeIDE & Latest & Beacon/receiver firmware modification \\
