...
\begin{document}
	...
	\begin{table}[ht!]
		...
		\begin{tabularx}{\textwidth}{l c c X}
			...
			\texttt{rst\_n} & in & 1 & Active-low reset (mapped from \texttt{CPU\_RESETN}) \\
			\texttt{rx} & in & 1 & Raw \UART{} serial input (asynchronous) \\
			\texttt{rx\_data} & out & 8 & Received byte (valid when \texttt{rx\_strobe} = `1') \\
			\texttt{rx\_strobe} & out & 1 & One-cycle pulse indicating a valid byte is available \\
			\bottomrule
		\end{tabularx}
	\end{table}
	...
	\begin{table}[ht!]
		...
		\begin{tabularx}{\textwidth}{l c c X}
			...
			\midrule
			\texttt{clk} & in & 1 & 100~MHz system clock \\
			\texttt{rst\_n} & in & 1 & Active-low reset \\
			\texttt{fifo\_data} & in & 8 & Byte from receive \FIFO{} \\
			\texttt{fifo\_empty} & in & 1 & \FIFO{} empty flag \\
			\texttt{fifo\_rd\_en} & out & 1 & \FIFO{} read enable (parser requests next byte) \\
			\texttt{feat\_busy} & in & 1 & Backpressure from feature extraction datapath \\
			\texttt{metadata} & out & 64 & Parsed frame metadata bytes 2--9 \\
			\texttt{payload} & out & 256 & Parsed radio packet bytes 10--41 \\
			\texttt{rx\_crc} & out & 8 & Captured frame \CRC{}-8 byte (offset 42) \\
			\texttt{frame\_valid} & out & 1 & One-cycle pulse: complete frame passed checks \\
			\texttt{frame\_reject} & out & 1 & One-cycle pulse: frame failed checks \\
			\bottomrule
		\end{tabularx}
	\end{table}

	The implementation used in Lab Handouts~2-D through 2-F keeps the parser type-agnostic and exports packed \texttt{metadata}/\texttt{payload}; bit-field extraction (including \texttt{pkt\_type} and PAUSE-specific fields) is performed in the feature extraction stage.
	...
	\begin{tcolorbox}[warning={Common Pitfalls}]
		\begin{itemize}
			\item \textbf{First-observation edge case:} In the reference LH2 implementation, previous timestamp and RSSI initialize to zero. Therefore, first-observation Inter-Arrival Time resolves to \texttt{current\_timestamp - 0}, and \RSSI{} Delta resolves to \texttt{current\_rssi - 0}. If a sentinel scheme is preferred, it must be added consistently in both \VHDL{} and the Python reference model.
	...
	With Beacon~IDs ranging from \texttt{0x01} to \texttt{0xFF} (255 possible senders), the logical per-station state footprint is $255 \times 7 = 1{,}785$ bytes. In the LH2 reference implementation, these state fields are stored inside each station's 32-byte feature row and updated via read-modify-write, so no separate state-RAM macro is instantiated.

	\begin{tcolorbox}[infobox={Design Choice: Logical State vs. Separate State RAM}]
		For the expected benchtop scenario of 2--3 active stations (Target~A, Target~B, and occasionally the Threat), either a dedicated state RAM or feature-row read-modify-write can work. The LH2 implementation chooses read-modify-write on the feature \BRAM{} so state scales to the full 255-sender space without consuming additional \BRAM{} blocks beyond the feature store itself.
	\end{tcolorbox}
	...
	\begin{table}[ht!]
		...
		\begin{tabular}{l l l}
			...
			Total Size & 8~KB & Fits in 2$\times$ 36~Kb \BRAM{} tiles \\
			Port A & Write (\FPGA{} pipeline) & Synchronous write on \texttt{frame\_valid} \\
			Port B & Read (host interface) & Synchronous readback for SW/LED and simulation dump paths \\
			\bottomrule
		\end{tabular}
	\end{table}
	...
	\begin{enumerate}[leftmargin=2em]
		\item Reads the current feature row for the incoming Beacon~ID and uses its prior timestamp/\RSSI{}/count fields as the per-station state.
		...
		\item Updates the same row's state fields (timestamp, \RSSI{}, incremented count) as part of that write.
	\end{enumerate}

	This sequence completes in a small fixed number of clock cycles (row read latency + computation + write = $\sim$4--6 cycles at 100~MHz), which is negligible compared to the $\sim$3.73~ms frame arrival interval at 115200 baud.

	\subsubsection{Read / Export Interface}

	For Module~2, the implemented readback path is switch/LED based on the Nexys board: \texttt{SW(7:0)} selects the Beacon-ID row, \texttt{SW(12:8)} selects one of 32 bytes within that row, and \texttt{LED(15:8)} displays the selected byte. Full-row verification in simulation uses the debug readback signal exported by \texttt{ingest\_top}. The full host export protocol is finalized during Module~3 integration.

	\subsection{Station Prioritization Logic}

	As a stretch goal for advanced students, implement a simple prioritization metric that ranks active stations by signal quality. In the LH2 reference implementation, the score is:

	\begin{equation}
		P^{(k)} = 4\cdot \text{\RSSI{}}^{(k)} + 1\cdot \text{\SNR{}}^{(k)} + \text{frame\_count}^{(k)}
		\label{eq:priority}
	\end{equation}

	In early Module~2 bring-up stages, seven-segment output is used for quick visual checks (continuing the LH1-H pattern). In the production LH2-F datapath, seven-segment driving is retired and readback moves to the switch/LED interface; this staged transition is intentional and part of the curriculum flow.
	...
	\begin{figure}[ht!]
		...
		\resizebox{\textwidth}{!}{%
			\begin{tikzpicture}[
				...
				\node[block, right=of bram] (export) {Host\\Readback\\(SW/LED + Sim)};

				% Logical prior-state fields (stored in the feature BRAM row)
				\node[mem, below=1.5cm of feat] (state) {Prior-State\\Fields\\(in BRAM row)};
				...
				% Arrows
				\draw[arrow] ($(sync.west)+(-1,0)$) -- (sync.west) node[midway, above, elabel] {uart\_rx\_pin};
				...
				\draw[arrow] (export.east) -- ++(1,0) node[right, font=\scriptsize] {To board indicators / simulation checker};
		...
		\caption{Complete Module~2 \FPGA{} pipeline architecture. Solid blocks are implemented across Sessions~5--7. Readback in this module uses the SW/LED path and simulation dump interface; the full host export protocol is finalized in Module~3.}
	...
	\begin{tcolorbox}[deliverable={Session 7 Deliverable}]
		...
		\begin{enumerate}
			...
			\item \VHDL{} source code for the feature extraction module and feature \BRAM{} (including row-level prior-state read-modify-write behavior).
	...
	\begin{table}[ht!]
		...
		\begin{tabularx}{\textwidth}{l c c c X}
			\toprule
			\textbf{Module} & \textbf{\LUT{}s} & \textbf{FFs} & \textbf{\BRAM{} (36Kb)} & \textbf{Notes} \\
			\midrule
			...
			Per-Station State Storage & 0 & 0 & 0 & Stored in feature row fields (read-modify-write) \\
			Feature \BRAM{} & 0 & 0 & 2 & 256$\times$256 bits \\
			Readback Mux (SW/LED) & $\sim$20 & $\sim$20 & 0 & Byte-selection logic \\
			\midrule
			\textbf{Total Estimate} & $\sim$\textbf{550} & $\sim$\textbf{620} & \textbf{2} & \\
			\textbf{Artix-7 100T Available} & 63,400 & 126,800 & 135 & \\
			\textbf{Utilization} & $<$1\% & $<$1\% & $\sim$1--2\% & \\
			\bottomrule
		\end{tabularx}
	\end{table}
	...
	\begin{table}[ht!]
		...
		\begin{tabularx}{\textwidth}{l l l X}
			...
			\midrule
			\texttt{uart\_rx\_pin} (from STM32) & JA[1] & PMOD JA & Interceptor \MCU{} TX $\rightarrow$ \FPGA{} RX \\
			\texttt{UART\_RXD\_OUT} (to host) & D4 & USB-\UART{} & Raw-stream passthrough / capture path \\
			\texttt{CLK100MHZ} & E3 & On-board & 100~MHz system clock \\
			\texttt{CPU\_RESETN} & C12 & BTNC & Center pushbutton (active-low reset) \\
			\texttt{SW[15:0]} & J15--R17 & On-board switches & Row and byte selector for feature readback \\
			\texttt{LED[15:0]} & H17--V14 & On-board LEDs & Debug pulses + selected feature byte on \texttt{LED(15:8)} \\
			\texttt{CA..CG, DP, AN[7:0]} & Various & 7-seg display & Used in early bring-up checks; forced inactive in LH2-F production datapath \\
			\bottomrule
		\end{tabularx}
	\end{table}
	...
	\begin{table}[ht!]
		...
		\begin{tabularx}{\textwidth}{X c c}
			...
			Feature Extraction Datapath Design Document (feature vector with \texttt{pkt\_type} column, row-level state tracking, \BRAM{}, multi-station) & 25\% & Session 7 \\
