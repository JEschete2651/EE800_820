...
\begin{document}
	...
	\begin{titlepage}
		...
		\vspace{0.5cm}
		{\LARGE\color{darkgray} FPGA RTL Design, Signal Ingestion Pipeline,\\[4pt]
		...
		\begin{tcolorbox}[colback=lightgray, colframe=darkgray, width=0.85\textwidth, arc=2pt]
			...
			covering the design and simulation of the FPGA-side signal processing pipeline on the
			Digilent Nexys A7-100T. Students implement UART-based data ingestion, packet parsing
			with hardware CRC verification, and a feature extraction datapath in VHDL, then verify
	...
	\begin{tcolorbox}[objective={Module 2 Learning Objectives}]
		...
		\begin{enumerate}[label=\textbf{LO-\arabic*}]
			...
			\item Architect a feature extraction datapath that transforms parsed packet fields into a structured feature vector suitable for the machine learning classification pipeline in Module~4.
			...
			\item Analyze FPGA resource utilization (LUTs, flip-flops, block RAM, DSP slices) and timing reports to make informed design trade-offs within the Artix-7 fabric constraints.
		\end{enumerate}
	\end{tcolorbox}

	\subsection{Context Within the Project}

	Module~2 builds directly on the hardware platform validated in Module~1. The UART interface between the receiver STM32 and the Nexys A7 FPGA, characterized in Module~1 Week~4, becomes the input to the RTL processing pipeline developed here. The beacon packet format (32~bytes, Table~4 in Module~1) and the receiver-to-FPGA forwarding frame (43~bytes, Table~6 in Module~1) define the exact data structures that the FPGA logic must parse.
	...
	\begin{itemize}[leftmargin=2em]
		...
		\item Understanding of synchronous digital design principles: clock domains, setup/hold timing, finite state machines, and registered outputs.
	...
	\begin{table}[ht!]
		...
		\begin{tabularx}{\textwidth}{l c c c X}
			\toprule
			\textbf{Module} & \textbf{LUTs} & \textbf{FFs} & \textbf{BRAM (18Kb)} & \textbf{Notes} \\
			\midrule
