...
\end{equation}
The FPGA pipeline contribution is bounded above by six clock cycles of the 100~MHz ingestion datapath after the last byte of the frame, well under 0.1~ms. The receiver firmware ISR plus frame construction adds under~1~ms. The RF on-air time dominates and depends strongly on the LoRa spreading factor; for a 32-byte payload at $B=125$~kHz, CR=4/5, the SX1276 airtime formula~\cite{b12} yields the values in Table~\ref{tab:m3latency}.

\begin{table}[t]
\centering
\caption{Module~3 closed-form end-to-end latency budget for a 32-byte beacon payload at $B=125$~kHz, CR=4/5.}
\label{tab:m3latency}
\begin{tabular}{l c c c}
\toprule
\textbf{SF} & \textbf{$L_{\mathrm{RF}}$ (ms)} & \textbf{$L_{\mathrm{e2e}}$ (ms)} & \textbf{RF share} \\
\midrule
SF7  & $\sim$50    & $\sim$55    & $\sim$91\% \\
SF10 & $\sim$370   & $\sim$375   & $\sim$99\% \\
SF12 & $\sim$1500  & $\sim$1505  & $\sim$99.7\% \\
\bottomrule
\end{tabular}
\end{table}
...
The five single-variable sweep campaigns of Module~3 (Lab Handouts~3-F and 3-G) produce a labeled feature dataset at known target volume. Table~\ref{tab:m3dataset} tabulates the target packet count per campaign, the resulting raw row count, and the projected effective row count after the Module~1 acceptance bound of $\leq$5\% PER (Section~\ref{sec:m3accept}) is applied at each level.

\begin{table}[t]
\centering
\caption{Module~3 campaign volume across Lab Handouts~3-F and 3-G, with effective row count under the $\leq$5\% PER bound.}
\label{tab:m3dataset}
\begin{tabular}{l l c r r}
\toprule
\textbf{Campaign} & \textbf{Sweep} & \textbf{Steps} & \textbf{Pkts} & \textbf{Eff.\ rows} \\
\midrule
C1 & TX power            & 4 & 800   & $\geq$760  \\
C2 & Spreading factor    & 6 & 1{,}200 & $\geq$1{,}140 \\
C3 & Modulation type     & 2 & 600   & $\geq$570  \\
C4 & Distance            & 4 & 800   & $\geq$760  \\
C5 & Beacon count / mode & 5 & 1{,}500 & $\geq$1{,}200 \\
\midrule
\textbf{Total} & --- & 21 & \textbf{4{,}900} & $\geq$\textbf{2{,}500} \\
\bottomrule
\end{tabular}
\end{table}

The aggregate target of 4{,}900 transmitted packets is projected to yield at least 2{,}500 effective feature vectors after PER attrition, which is the minimum threshold below which the stratified 70/15/15 split of Lab Handout~3-H cannot guarantee at least 30 test-set samples per minority class. This in turn drives the per-level packet-count choices in Lab Handouts~3-F and~3-G: 200 packets per level at C1, C2, and C4, and 300 per level at C3 and C5, are not arbitrary collection durations but the smallest values that satisfy the downstream split constraint at the worst-case PER allowance.
...
Module~4 trains four classifier families per task, summarized in Table~\ref{tab:m4classifiers}. The grid for the random forest spans $3 \times 3 \times 2 \times 2 = 36$ hyperparameter configurations evaluated with 5-fold stratified cross-validation over the concatenated train+validation split; this concatenation is necessary because at the framework's projected $\sim$2{,}500-row dataset size, the bare 15\% validation split holds only $\sim$375 rows, which is too small a sample for reliable hyperparameter selection. The MLP and CNN-1D are trained with Adam at learning rate $10^{-3}$ and batch size 32, with early stopping on the validation cross-entropy loss at a patience of 15 epochs.
...
\begin{table}[t]
\centering
\caption{Module~4 end-to-end response time budget at the SF7 baseline configuration, extending Table~\ref{tab:m3latency} through host inference.}
\label{tab:m4latency}
\begin{tabular}{l c l}
\toprule
\textbf{Stage} & \textbf{Budget (ms)} & \textbf{Source} \\
\midrule
RF on-air (SF7, 32~B)              & $\sim$50    & \S\ref{sec:m3latency} \\
Receiver firmware ISR              & $<$0.5      & \S\ref{sec:m3latency} \\
UART transport (43~B, 115{,}200)   & 3.73        & \S\ref{sec:m3latency} \\
FPGA pipeline                      & $<$0.1      & \S\ref{sec:results}.D \\
Host inference (3 tasks)           & $<$6        & LH4-H target \\
\midrule
\textbf{Total end-to-end}          & $<$60       & --- \\
\bottomrule
\end{tabular}
\end{table}
...
This paper presented an educational framework that treats the interfaces between FPGA digital design, RF communications, embedded firmware, and machine learning as first-class teaching objects. The framework is instantiated by a benchtop platform built from commercially available components---STM32 Nucleo-L476RG microcontrollers, RFM95W LoRa transceivers, and a Digilent Nexys A7-100T FPGA---and a five-module progressive curriculum that sequences students through hardware bring-up, RTL design, system integration, AI-driven classification, and full-system benchmarking. A Python engagement simulator with a physics-grounded FSPL link model and known ground-truth labels bridges the gap between curriculum introduction and hardware availability, and later serves as the controlled baseline against which real-hardware machine learning results are compared. Quantitative analysis of the first four fully instantiated modules shows that the chosen hardware leaves more than 100~dB of link margin at bench distances while still exposing every LoRa physical-layer parameter as a tunable student-facing variable, that the Module~1 FPGA ingestion datapath consumes under 0.5\% of the Artix-7 \texttt{xc7a100t} LUT budget and leaves the entire DSP and block-RAM resource pool available for the classification pipeline added in Modules 2 and~4, that the complete Module~2 ingestion pipeline closes timing at the target 100~MHz with worst negative slack greater than~3~ns and uses approximately 2\% of available block RAM, and that the engagement simulator's closed-form performance envelope provides a rigorous overfitting check for student-trained classifiers. For Module~3, the closed-form latency budget shows that RF on-air time accounts for $\sim$91\% of end-to-end latency at SF7 and $\sim$99.7\% at SF12, isolating the system's optimization headroom to the RF subsystem rather than the digital processing pipeline; the 268~fps UART throughput ceiling sits well above the operating envelope of three concurrent beacons, with the 64-entry receive FIFO retaining headroom even under maximum-contention ALOHA arrivals; and the five sweep campaigns of Lab Handouts~3-F and~3-G target 4{,}900 transmitted packets across 21 collection runs, projected to yield at least 2{,}500 effective feature vectors with stratified train/val/test splits keyed on \texttt{run\_id}. For Module~4, the dataset is gated by a ten-check audit that runs before any preprocessing, three classification tasks proceed on disjoint feature sets derived from a label-leakage table that a logistic-regression sanity check independently validates at $\geq$0.95 accuracy on forbidden columns alone, four classifier families (DummyClassifier, decision tree, grid-searched random forest, fully-connected MLP, and 1-D CNN over 8-row beacon-grouped sequences) are compared with 95\% Wilson confidence intervals to make significance claims defensible at $n\!\approx\!375$, the sim-to-real generalization gap $\Delta = \mathrm{Acc}(\mathrm{hw}\!\to\!\mathrm{hw}) - \mathrm{Acc}(\mathrm{sim}\!\to\!\mathrm{hw})$ is measured per task and partially closed by fine-tuning only the final linear layer on 10--20\% of hardware data, and the multi-task real-time inference engine targets a 99th-percentile latency under 6~ms against a 996~ms inter-packet budget while a schema manifest enforces the LH4-A data contract at deployment time as well as at training time. The Module~1 skeleton deliberately consumes only a small fraction of the Artix-7 device so that the CRC-16 checker, feature extraction datapath, and streaming inference blocks added in Modules~2 and~4 can be introduced as student additions rather than replacements---preserving the framework's integration-first pedagogy through the entire module sequence. Taken together, these results demonstrate that the framework is not only pedagogically coherent but also technically feasible within the resource and complexity budgets of a graduate-level capstone course, and that an instructor adopting the framework at another institution can reproduce both the hardware platform and the quantitative acceptance criteria directly from the specification given here.
